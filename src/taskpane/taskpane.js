...
Office.onReady(info => {
  if (info.host === Office.HostType.Word) {
    ...
    if (!Office.context.requirements.isSetSupported('WordApi', '1.3')) {
        document.getElementById("error-box").innerHTML += "Sorry. The tutorial add-in uses Word.js APIs that are not available in your version of Office.<br/>";
    }
    ...
    document.getElementById("link-hero").onclick = function() {changeCitations("hero")};
    document.getElementById("link-heronet").onclick = function() {changeCitations("heronet")};
...
function changeURL(url, oldURL) {
    ...
    if (!oldURL.includes(heroURL) && !oldURL.includes(heroNetURL)) {return null;}
    if (url == "hero") {
...
function writeString(context, string) {
    // for debugging
    var docBody = context.document.body;
    docBody.insertParagraph("" + string,
                            "End");
}
...
function fixProblems(context, url, oParser, oldXML, bookmarkList, citationMatching, citationList) {
    ...
    if (newXML !== null) {
        ...
        for (var ref in bookmarkList) {
            if (!Object.prototype.hasOwnProperty.call(bookmarkList, ref)) {continue;}
...
function changeFirstBibEntry(context, url, oParser, oldXML, bookmarkList, citationMatching, citationList) {
    ...
    if (!oldXML.includes(thisRef)) {return null;}
    ...
    for (var j = 0; j < citeSplit.length; j++) {
        ...
        if (searchMatch.includes("(")) {parenCount = parenCount + 1;}
        if (searchMatch.includes(")")) {parenCount = parenCount - 1;}
        runningCount = runningCount + searchMatch.length;
        if (searchMatch.length < 4) {continue;}
        if (searchMatch.match(/[(][^)]{4,}[)][.]?/)) {break;}
        if (j > 4) {break;}
        if (runningCount > 40 && parenCount == 0) {break;}
    }
    ...
    if (runningCount > 0) {
        for (var q = 0; q < j+1; q++) {
            var delim = "";
            if (q < citeSplit.length-1) {delim = ".";}
            combRange = combRange + citeSplit[q] + delim;
        }
    }
    if (combRange.length == 0) {
        document.getElementById("error-box").innerHTML += '<p class="p-warn"><span class="style-err">' +
    ...
    for (var a = 0; a < bookmarkList2.length; a++) {
        ...
        if (bookmark.hasAttribute("w:name") && bookmark.getAttribute("w:name") == "_ENREF_1") {
            ...
            if (thisParagraph.nodeName != "w:p" || !thisParagraph.hasAttribute("w:rsidRPr")) {continue;}
            var rsid = thisParagraph.getAttribute("w:rsidRPr");

            var newText = "" +
            '<w:r><w:fldChar w:fldCharType="begin"/></w:r>' +
            '<w:r><w:instrText xml:space="preserve"> HYPERLINK "' + newURL + '" </w:instrText></w:r>' +
            '<w:r><w:fldChar w:fldCharType="separate"/></w:r>' +
            '<w:r w:rsidRPr="' + rsid + '"><w:rPr><w:rStyle w:val="Hyperlink"/></w:rPr>' +
            '<w:t>' + combRange + '</w:t></w:r>' +
            '<w:r><w:fldChar w:fldCharType="end"/></w:r>';

            var splitStr = '<w:bookmarkStart w:id="0" w:name="_ENREF_1"/>';
    ...
    if (finalXMLstr.length == 0) {
        document.getElementById("error-box").innerHTML += '<p class="p-warn"><span class="style-err">' +
            'Could not add a link to the first bibliograpgy entry.</span></p>';
        return null;
    }
...
function writeBibLinks(context, url, bibSearch3, citationMatching, citationList) {
    ...
    for (var ref in bibSearch3) {
        if (!Object.prototype.hasOwnProperty.call(bibSearch3, ref)) {continue;}
...
function findBibTextToLink(context, bibSearch2) {
    ...
    for (var ref in bibSearch2) {
        if (!Object.prototype.hasOwnProperty.call(bibSearch2, ref)) {continue;}
        var searchCollectionList = bibSearch2[ref];

        var searchList = [];
        for (var i = 0; i < searchCollectionList.length; i++) {  // should only have length of 1
            ...
            for (var j = 0; j < searchCollection.items.length; j++) {
                ...
                if (searchMatch.text.includes("(")) {parenCount = parenCount + 1;}
                if (searchMatch.text.includes(")")) {parenCount = parenCount - 1;}
                runningCount = runningCount + searchMatch.text.length;
                if (searchMatch.text.length < 4) {continue;}
                if (searchMatch.text.match(/[(][^)]{4,}[)][.]/)) {break;}
                if (j > 4) {break;}
                if (runningCount > 40 && parenCount == 0) {break;}
            }
            ...
            if (runningCount > 0) {
                ...
                document.getElementById("error-box").innerHTML += '<p class="p-warn"><span class="style-err">' +
                    'Could find text to add ref ' + ref + ' to a bibliography entry.</span></p>';
            }
        }
...
function organizeBibText(context, bibSearch) {
    ...
    for (var ref in bibSearch) {
        if (!Object.prototype.hasOwnProperty.call(bibSearch, ref)) {continue;}
        var searchList = [];
    ...
    for (var n = 0; n < linkRanges.items.length; n++) {
        ...
        if (oldURL in citationMatching && oldURL != oldText) {
            ...
            if (changedURL !== null) {
                ...
                document.getElementById("error-box").innerHTML += '<p class="p-warn"><span class="style-' + errStyle + '">' +
                    'Hyperlink "' + oldText + '" ("' + oldURL + '")' + ' not changed.</span></p>';
            }
        }
    }
}
...
function findMatchingCitation(context, citationList, bookmarkList) {
    ...
    for (var ref in bookmarkList) {
        ...
        for (var a = 0; a < citationList.length; a++) {
            if (usedCitations.includes(a)) {continue;}
            var thisCitation = citationList[a];
            var checkPass = 0;

            // check for matching text
            if (thisCitation.author != "" && thisBib.includes(thisCitation.author)) {checkPass += 2;}
            if (thisCitation.year != "" && thisBib.includes(thisCitation.year)) {checkPass += 1;}
            if (thisCitation.title != "" && thisBib.includes(thisCitation.title)) {checkPass += 4;}
        ...
        if (citeInd == -1) {
            document.getElementById("error-box").innerHTML += '<p class="p-warn"><span class="style-err">' +
                'Could not find a matching citation for bib entry "' + thisBib.split(".")[0] + '...".</span></p>';
        } else {
            if (tieBreak < 7) {
                document.getElementById("error-box").innerHTML += '<p class="p-warn"><span class="style-warn">' +
                    'Matched citation for bib entry "' + thisBib.split(".")[0] + '...", but it was not a full match.</span></p>';
            }
...
function getBibCitations(context, xmlDOM) {
    ...
    for (var a = 0; a < bookmarkList.length; a++) {
        ...
        if (bookmark.hasAttribute("w:name")) {
            ...
            if (!anchorName.includes("_ENREF_")) {continue;}
...
function getCitationList(context, xmlDOM) {
    ...
    if (extraGet === null) {extraGet = [];}
    if (extraGet.length > 0) {decodeList.push(extraGet[0].textContent);}
    var sameCt = 1;
